...
const Login = ({ setShowLogin }) => {
    ...
    return (
        <div onClick={()=> setShowLogin(false)} className='fixed top-0 bottom-0 left-0 right-0 z-100 flex items-center text-sm text-gray-600 bg-black/50'>
            ...
            <div className="w-full">
              <p>Email</p>
              <input onChange={(e) => setEmail(e.target.value)}   value={email} placeholder="Type here" required
              className="border border-gray-200 rounded w-full p-2 mt-1 outline-primary" type="email" />
            </div>
